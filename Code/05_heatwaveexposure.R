...
p_load(dplyr, tidyr, readr, readxl, writexl, purrr, ggplot2, forcats, stringr, 
       patchwork, ggrepel, geomtextpath, colorspace, Hmisc, stars, ncmeta)
...
iso3c_tbl <- read_csv(here("Data", "countrygroups", "iso3c_region_mapping_20240319.csv"), show_col_types = FALSE) %>% 
  mutate(r10 = r10_iamc) %>% 
  select(iso3c, r10) %>% 
  right_join(read_csv(here("Data", "processed", "02_iso3c_indicators.csv")) %>% distinct(iso3c))

# Set consistent r10 ordering
r10order <- tibble(r10 = c("R10NORTH_AM", "R10EUROPE", "R10PAC_OECD", "R10REF_ECON", "R10CHINA+", 
                           "R10MIDDLE_EAST", "R10REST_ASIA", "R10LATIN_AM", "R10AFRICA", "R10INDIA+"),
                   ...
                   r10labellong = c("North America", "Europe", "Asia-Pacific Developed",
                                    ...
                                    "Eastern Asia", "North Africa and Middle East", 
...
# Read in netcdf file to stars tbl_cube
exp_heatwave <- stars::read_ncdf(here("Data", "impacts", 
                                      "nc_lifetime_exposure_new_heatwavedarea.nc4"), 
                                 var = "lifetime_exposure") %>%
...
exp_heatwave_r10 <- exp_heatwave %>%
  ...
  mutate(
    Scenario = trimws(GMT),  
    model = ifelse(grepl(GMT, pattern = "MESSAGE"), "MESSAGEix-GLOBIOM", "REMIND-MAgPIE"),
    case = case_when(
        grepl("Median", GMT) &
        grepl("incrate2", GMT) &
        !grepl("NPi\\|", GMT) ~ "A",
        grepl("Median", GMT) &
        grepl("incrate3", GMT) &
        grepl("nz_all_GHG", GMT) &
        grepl("cert_allconf_0.1", GMT) ~ "E",
      grepl("DeepElec", GMT) ~ "IMP-REN"),
    quantile = str_extract(GMT, "(?<=_)[^_]*$")) %>%
  filter(case %in% c("A", "E", "IMP-REN"), model == "REMIND-MAgPIE") %>% # Model used to match IMP-REN
  group_by(birth_year, r10, case, gcm, quantile, run) %>%
...
left_join(exp_heatwave_r10, exp_heatwave_r10_impren) %>% 
  ...
  mutate(
    case = case_when(
      ...
      case == "E" ~ "CurPledge",
      TRUE ~ case),
    case = factor(case, levels = c("CurPol", "CurPledge", "IMP-REN")),
...
a <- left_join(exp_heatwave_r10, exp_heatwave_r10_impren) %>% 
  ...
  geom_density(aes(x = add)) +
  scale_colour_discrete_qualitative(drop = F, 
                                    breaks = c("CurPol", "CurPledge")) +
  facet_wrap(~r10, ncol = 5) +
  theme_bw() +
  theme(legend.position = "top") +
  labs(y = NULL,
...
write_csv(exp_heatwave_r10_emf_impren, here("Data", "processed", "12_r10_exp_heatwave_emf.csv"))
