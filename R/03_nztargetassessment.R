...
nztargets <- read_xlsx(here("Data", "pathways", "zerotrackernet", "data_202403.xlsx"), sheet = 1) %>% 
...
# CAT net-zero target years
catnztargets <- read_xlsx(
  here("Data", "pathways", "zerotrackernet", "Analysis_update_ImplementationProgress_EGR2023_master_finalUpdateCheck.xlsx"), 
  skip = 2) %>% 
...
# Compare CAT NZ targets to NZT targets
...
a <- alltargets %>% 
  ...
  summarise("CO2FFI (GtCO2, 2019)" = sum(gtco2),
