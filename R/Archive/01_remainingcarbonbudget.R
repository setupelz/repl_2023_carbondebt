...
# Recent production-based CO2-FFI emissions - Global Carbon Budget 2023
# https://zenodo.org/records/10177738
...
final_iso3c <- 
  ...
  reduce(inner_join) %>% 
  mutate(countryname = countrycode::countrycode(iso3c, origin = "iso3c", destination = "country.name"))
...
iso3c_missing <- left_join(iso3c_missing, 
                           (read_csv(here("data", "equity_data",
                              ...
                              filter(year == 2022))) %>% 
  left_join(popproj) %>% 
  mutate(gtco2share = gtco2 / (sum(recent_prodco2$gtco2[recent_prodco2$year == 2022], na.rm = T)),
         gtco2share = scales::percent(gtco2share, accuracy = .01),
         popshare = pop / sum(popproj$pop[popproj$year == 2020], na.rm = T),
         popshare = scales::percent(popshare, accuracy = .01),
         year = 2022, 
         countryname = countrycode::countrycode(iso3c, "iso3c", "country.name")) %>% 
  select(r10, iso3c, countryname, year, gtco2, gtco2share, pop, popshare)
